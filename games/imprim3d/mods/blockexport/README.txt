...
This mod defines a new chat command "/blockexport" which writes 3D node data
of the player's current 200x100x200 node block to a file.  The chat command
...
Commands:
   ...
   /blockexport
      -- Requires "export" privilege.  Takes no parameters.  Exports nodes from
         the player's current 200x100x200 node block named in EXPORT_NODES.

Note: The default configuration has a soft dependency on wool simply because it
lists some "mesecons_lightstone:lightstone_red_off" node types as the ones to export.  See the EXPORT_NODES
