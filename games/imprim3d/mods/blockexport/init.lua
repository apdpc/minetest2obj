...
local FILE_PREFIX = "redmesecons2obj_";
local FILE_SUFFIX = ".obj";
local EXPORT_NODES = { "mesecons_lightstone:lightstone_red_off" };
...
local function hashVert(pos)
   return (pos.x or 0) + 200*(pos.y or 0) + 6400*(pos.z or 0);
end
...
local function blockExport(pos)
   local xMin = 200 * math.floor(pos.x / 200);
   local yMin = 100 * math.floor(pos.y / 100);
   local zMin = 200 * math.floor(pos.z / 200);
   local xMax = xMin + 199;
   local yMax = yMin + 99;
   local zMax = zMin + 199;
...
minetest.register_chatcommand(
   ...
   {
      params = "",
      description = "Exports your current 200x100x200 node block to a file",
      privs = { export = true },
      func =
         function(name, paramStr)
            local player = minetest.get_player_by_name(name);
            local fname = blockExport(player:getpos());
